...
IFloodlightModule {
	...
	public boolean isCallbackOrderingPostreq(OFType type, String name) {
		System.out.println("firewallllllllllllllllllllllllllllllllllllllllllllllllllllllllllllllll");
		//return (type.equals(OFType.PACKET_IN) && (name.equals("forwarding")));
		return (type.equals(OFType.PACKET_IN) && (name.equals("forwarding")|| name.equals("forwarding2")));
	}
	...
	protected RuleMatchPair matchWithRule(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
        ...
        logger.info("*****************match incoming flow with firewall rule*******************");
		synchronized (rules) {
			Iterator<FirewallRule> iter = this.rules.iterator();
			...
			if (rule == null || rule.action == FirewallRule.FirewallAction.DROP) {
				...
				if (logger.isTraceEnabled()) {
					if (rule == null) {
						...
						System.out.println("NO firewall rule found for PKTIN, block flows  ");
					} else if (rule.action == FirewallRule.FirewallAction.DROP) {
						logger.trace("Deny rule={} match for PacketIn={}", rule, pi);
						System.out.println("Deny rule={} match for PacketIn={} ");
					}
				}
				...
				decision = new RoutingDecision(sw.getId(), inPort, 
				      ...
				      System.out.println("Found a rule and the rule is not a drop, so allow the packet in FIREWALL ");
