...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener {
    ...
    long Endtime = 0;
    long StartFloodtime = 0;
    long floodtime = 0;
    String reqSw2 = "";
    
	@Override
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx, String reqSw, long Endtime) {
		Ethernet eth = IFloodlightProviderService.bcStore.get(cntx, IFloodlightProviderService.CONTEXT_PI_PAYLOAD);
		System.out.println("forwarding###########################"+eth +"\n"+"################################end of eth payload"+"\n");
		// We found a routing decision (i.e. Firewall is enabled... it's the only thing that makes RoutingDecisions)
		System.out.println("reqSw-forwarding from forwardingBase:"+ reqSw);
		System.out.println("Endtime from forwardingBase:"+ Endtime);
		// get srcMac and DestMac
		if(eth.getDestinationMACAddress().toString().contentEquals("ff:ff:ff:ff:ff:ff" ) == false && ( reqSw.equals(reqSw2 ))  ){
			System.out.println("Dest MAC  "+ eth.getDestinationMACAddress().toString());
			System.out.println("Dest host ACKed ARP");
			System.out.println("reqSw-forwarding from forwardingBase::"+ reqSw);
			System.out.println("Processing time ="+(Endtime-StartFloodtime));
			//Date date = new Date();
		    //Endtime = System.currentTimeMillis();
	        //System.out.println("Packet in REceived time: "+Endtime + "for switch "+sw.toString() );
		}else if ( (eth.getDestinationMACAddress().toString().contentEquals("ff:ff:ff:ff:ff:ff" ) == true)  ){
			System.out.println("Dest MAC  "+ eth.getDestinationMACAddress().toString());
			System.out.println("Flooding All ports");
			System.out.println("reqSw-forwarding from forwardingBase::"+ reqSw);
			//Date date = new Date();
		    
	        //System.out.println("Packet in REceived time: "+Endtime + "for switch "+sw.toString() );
	       // System.out.println("Packet out Delivered time: "+Endtime + "for switch "+sw.toString() );
		}
		...
		if (decision != null) {
			...
			System.out.println("heeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeelo");
			System.out.println("---------------------------->decision in Forwarding:"+decision.getRoutingAction().toString());
	...
	protected void doForwardFlow(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx, boolean requestFlowRemovedNotifn) {
		...
		if (dstDevice != null) {
			...
			System.out.println("Route in Forwarding "+route.toString());
			System.out.println("Route count in Forwarding :"+route.getRouteCount());
			//ArrayList<Route> getRoutes(DatapathId longSrcDpid, DatapathId longDstDpid, boolean tunnelEnabled);
			ArrayList allRoute = routingEngineService.getRoutes(source,dstDap.getSwitchDPID(),false);
			System.out.println("allRoute"+"\n"+allRoute.toString()+"\n");
			
			Route route2 = routingEngineService.getRoute(source, dstDap.getSwitchDPID(), U64.of(0));
			System.out.println("Route 2 in Forwarding: "+route2+"----end route2"+"\n");
			...
			if (route != null) {
				...
				System.out.println("No route.......Adding path now.....");
	...
	protected void doFlood(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		System.out.println("Write packet out with packetin data that is flooded to all ports");
		...
		try {
			...
			if(StartFloodtime == 0 && reqSw2 == ""){
				reqSw2 = sw.toString();
	             StartFloodtime= System.currentTimeMillis();
	             System.out.println("StartFloodtime: "+StartFloodtime + "for switch reqSw2-forwarding "+ reqSw2);
			}
	       // if(reqSw2 == ""){
	        // reqSw2 = sw.toString();
	       // }
			floodtime = System.currentTimeMillis();
	        System.out.println("Flood start time: "+floodtime + "for switch reqSw2-forwarding "+ sw.toString());
